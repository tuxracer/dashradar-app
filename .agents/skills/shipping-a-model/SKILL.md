...
Testing an unreleased revision on a device needs no code change at all: paste the revision-pinned weights URL into the model picker, behind the + in its corner. The picker's trial load runs the same download-build-run sequence as a real load, so it covers steps 2 and 5 on that one device; still verify thermals separately, and still bump `DEFAULT_MODEL` for the release itself.
