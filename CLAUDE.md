...
- **Two independent caches make the app work offline; clearing either forces a re-download.** Transformers.js caches model weights in the browser Cache API under its own store. The `"ort-runtime"` Workbox route (`vite.config.ts`) `CacheFirst`-caches the onnxruntime-web `.wasm`/`.mjs` that gets fetched at runtime from `cdn.jsdelivr.net` (onnxruntime-web always fetches from jsdelivr unless `wasmPaths` is configured, which it isn't here, so the ~23.5 MB `.wasm` Vite still emits into `dist/assets` is dead weight, deliberately excluded from the precache glob).
