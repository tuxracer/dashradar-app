...
- `src/lib/`: React-free domain modules, one directory each: `detection`, `detectionModels`, `detectionTracker`, `onnxMetadata`, `radarSignal`, `radarAudio`, `camera`, `crashSentinel`, `pwaInstall`, `saveFrame`, `serviceWorker`, `scanClock`, `timingHistory`, `browserEngine`, `appUpdate`, `resetAppData`, `wakeLock`, and friends.
