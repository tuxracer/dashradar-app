...
- **onnxruntime-web's WebGPU graph capture is fully wired but disabled (`WEBGPU_GRAPH_CAPTURE = false` in `src/workers/detection/consts.ts`); do not flip it on until the TopK blocker below is gone.** Graph capture records the model's kernel dispatches on the first run and replays them on later runs, which would cut the per-frame CPU dispatch overhead of RF-DETR's hundreds of small kernels. It requires every graph node to be partitioned onto the WebGPU EP. The model's v1.6 onnxslim re-export removed the shape subgraphs that originally tripped this check (1634 to 835 nodes, zero Shape/Cast/ConstantOfShape ops), but the graph still contains one `TopK` node, and onnxruntime-web 1.27 has no WebGPU (JSEP) kernel for TopK, so ORT assigns it to the CPU EP and `InferenceSession.create` still fails deterministically on every device with `This session cannot use the graph capture feature as requested by the user as all compute graph nodes have not been partitioned to the JsExecutionProvider`. Verified in Chrome via chrome-devtools MCP on the v1.6 exports: both `model.onnx` (fp32) and `model_fp16.onnx` fail identically (a model-repo changelog note claiming capture works on v1.6 did not reproduce). Because the failure is a property of the graph plus the ORT version, not the device, the attempt is gated off rather than shipped as try-and-fallback, which would waste a full failed fp32 session initialization on every WebGPU cold start. Everything else is in place behind the flag: `createCaptureModel` in the worker creates the session with `enableGraphCapture: true` and `preferredOutputLocation: "gpu-buffer"`, keeps the input in one persistent GPU buffer written per frame with `device.queue.writeBuffer` (device from `await env.webgpu.device` after session creation, tensor wrapped once via `Tensor.fromGpuBuffer`), reads outputs with `await tensor.getData(true)`, performs a validation-plus-warm-up run at load time (capture incompatibility can surface at the first `run()` rather than at session creation, and doing that run while the weights are still in scope makes the fallback to a plain WebGPU session cheap), and reports the outcome through the backend probe's `graphCapture`/`graphCaptureError` fields, shown in the debug overlay's "graph capture" row (`on`/`failed`/`disabled`/`n/a`). To enable capture for real: re-export the model without the TopK node (the app's decode scans all 300 queries itself, so an export that returns raw per-query outputs doesn't need TopK) or wait for an onnxruntime-web with a WebGPU TopK kernel, then flip the flag and confirm in Chrome that the overlay row reads "on" and detections match the uncaptured baseline.
