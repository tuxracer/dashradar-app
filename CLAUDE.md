...
A custom **RF-DETR Small** checkpoint fine-tuned on Las Vegas Metro police vehicles, published at [`tuxracer/las-vegas-metro-rfdetr-small`](https://huggingface.co/tuxracer/las-vegas-metro-rfdetr-small) and trained/exported from the sibling repo `~/Development/las-vegas-metro-rfdetr-small`. Mobile WebGPU is the only target and the only execution path, so a bad export has nothing to catch it. `src/lib/detectionModels` is the one place a checkpoint is named: one entry per selectable model, holding its `id` (what the stored selection is keyed on), Hugging Face `slug`, pinned `revision` (never `main`), and ONNX `file` (`model_fp16.onnx`, ~57 MB, mixed precision). `DEFAULT_MODEL` is the entry a build ships with. That is the whole entry: it says which bytes to fetch and nothing about what they contain, so pointing the detector at a new checkpoint is an entry, and a re-export is a one-line revision bump.

Everything about the head is read from the weights at load. The width comes off the `labels` output of the run the worker already performs, and the classes come from the `names` map the export stamps into the ONNX metadata (`classesFromMetadata`, read by `src/lib/onnxMetadata`): a map of logit index to label, which becomes the label a box carries. `displayLabelOf` in `src/lib/detection` uppercases it and opens separators up where the HUD draws it, so no derived copy travels with a detection. Labels and the logits they index therefore come from one file and cannot disagree, which is why nothing is declared here for a mismatch to be caught against. A file that names nothing still loads and detects, with generic per-slot labels, because the meter reads scores rather than words. The table stays in the worker, the only place a logit index means anything.
