...
Everything about the head is read from the weights at load. The width comes off the `labels` output of the run the worker already performs, and the classes come from the `names` map the export stamps into the ONNX metadata (`classesFromMetadata`, read by `src/lib/onnxMetadata`): a JSON map of logit index to label, which becomes the label a box carries and, uppercased with separators opened up, the text the HUD shows. Labels and the logits they index therefore come from one file and cannot disagree, which is why nothing is declared here for a mismatch to be caught against. A file that names nothing still loads and detects, with generic per-slot labels, because the meter reads scores rather than words. The worker derives the table and sends it to the main thread on `ready`, since only the worker sees the weights.
