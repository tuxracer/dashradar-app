...
- **Frame-pump invariants in `src/lib/detectionEngine` are hard-won race fixes**: one frame in flight (`inFlight`), a generation counter invalidating captures from before a stop, and a `workerLoaded` gate so a recycle's still-loading worker is never handed a frame. The engine's running state is derived from its inputs (video attached, page visible, settings closed), never commanded.
