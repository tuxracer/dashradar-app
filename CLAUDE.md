...
A custom **RF-DETR Small** checkpoint fine-tuned on Las Vegas Metro police vehicles, published at [`tuxracer/las-vegas-metro-rfdetr-small`](https://huggingface.co/tuxracer/las-vegas-metro-rfdetr-small) and trained/exported from the sibling repo `~/Development/las-vegas-metro-rfdetr-small`. Mobile WebGPU is the only target and the only execution path, so a bad export has nothing to catch it. `src/lib/detectionModels` is the one place a checkpoint is described: one entry per selectable model, holding its `id` (what the stored selection is keyed on), Hugging Face `slug`, pinned `revision` (never `main`), ONNX `file` (`model_fp16.onnx`, ~57 MB, mixed precision), `headWidth`, and `classes`. `DEFAULT_MODEL` is the entry a build ships with. Pointing the detector at a new checkpoint is an entry, not a set of edited constants. `headWidth` is the width of that checkpoint's classification head, background slot included, and is optional: the worker measures the real width off the `labels` output of the run it already performs at load (`resolveLoadedModel`), so declaring one is an assertion rather than a lookup. Registered entries declare it anyway, because the measured width agrees with whatever loaded and so cannot pin a table to its checkpoint; the declared width is what makes a table paired with the wrong checkpoint throw `MODEL_LOAD_FAILED` at load instead of mislabeling every box. `classes` names only the classes this build surfaces, each carrying its own logit index alongside the label the model outputs and how the HUD displays and colors it, so a checkpoint trained on 80 classes can expose the six that matter and a logit no entry names is never read. That does not make a new class free: `hudSignal` takes the max score across every detection regardless of class, so a new class immediately drives the dial, the `ALERT_THRESHOLD` ring, and the beeper, with no per-class alert concept to opt it out (a `person` class, for instance, would beep and flash red at every pedestrian). [docs/Models.md](docs/Models.md) is the contract a checkpoint has to meet to run here (tensor signature, head layout, precision, how to register it), and is where to send anyone asking what it would take to add a model. Read the `shipping-a-model` skill before changing the checkpoint, the export, the signature, or the revision.
...
- **Raw onnxruntime-web on purpose, never the Transformers.js `pipeline()`.** The head is a sigmoid scored per class, with an unused background slot at index 0 and a width the model's registry entry declares, and the pipeline's softmax-with-background DETR decode drops every real detection.
