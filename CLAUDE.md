...
- **`src/context/DetectionContext/`**: worker lifecycle and frame-pump state machine; consume with the `useDetection()` hook (status `loading-model → ready → running → error`, backend, model load progress, the latest `HudModel`, fps, a per-frame `debug` snapshot, `start`/`stop`). In production the worker's `load` message is deferred until the service worker controls the page (see Gotchas) so the first-visit model download is cached. The `debug` snapshot (`DebugSnapshot`) combines the worker's per-frame `timing` with context-side capture and round-trip timing plus raw/filtered detection counts, updated on every result regardless of whether the debug overlay is shown.
...
- **`src/components/`**: `CameraView` (the `<video>`), `RadarBackdrop` (static radar-grid background layer), `HudOverlay` (nearest-object box + floating tag markers; takes a `debug` prop that annotates each with its confidence and normalized box coords), `RadarStrip` (lane-radar blips), `StatusBar` (wordmark + settings gear), `SettingsButton` (enlarged gear that opens the full-screen settings panel), `SettingsScreen` (full-screen settings panel: Video feed and Debug overlay toggles plus read-only Detection engine, Model, and About rows), `DebugOverlay` (top-left diagnostics panel showing timing, detection counts, and system info, rendered only when the `showDebug` setting is on), `ModelLoadScreen` (download progress), `ErrorScreen` (camera/detection error copy with a retry action).
