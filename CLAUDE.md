...
- **`src/context/DetectionContext/`**: worker lifecycle and frame-pump state machine; consume with the `useDetection()` hook (status `loading-model → ready → running → error`, backend, model load progress, the latest `HudModel`, fps, `start`/`stop`). In production the worker's `load` message is deferred until the service worker controls the page (see Gotchas) so the first-visit model download is cached.
- **`src/context/SettingsContext/`**: localStorage-backed display options (`showVideo`) plus the ephemeral full-screen-settings open state (`settingsOpen`/`openSettings`/`closeSettings`, not persisted), consumed via the `useSettings()` hook. Persisted options are stored under `dashradar:settings` and validated on read. Wraps the app outside `DetectionProvider`.
...
- **`src/components/`**: `CameraView` (the `<video>`), `RadarBackdrop` (static radar-grid background layer), `HudOverlay` (nearest-object box + floating tag markers), `RadarStrip` (lane-radar blips), `StatusBar` (wordmark + settings gear), `SettingsButton` (enlarged gear that opens the full-screen settings panel), `SettingsScreen` (full-screen settings panel: Video feed toggle plus read-only Detection engine, Model, and About rows), `ModelLoadScreen` (download progress), `ErrorScreen` (camera/detection error copy with a retry action).
