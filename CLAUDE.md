...
- **onnxruntime-web's WebGPU graph capture (`enableGraphCapture: true`) cannot be enabled for this model export; don't re-try it without re-exporting the model first.** Graph capture records the model's kernel dispatches on the first run and replays them on later runs, which would cut the per-frame CPU dispatch overhead of RF-DETR's hundreds of small kernels. It requires every graph node to be partitioned onto the WebGPU EP, and ORT assigns some of this export's shape-related ops to the CPU EP, so `InferenceSession.create` fails deterministically on every device with `This session cannot use the graph capture feature as requested by the user as all compute graph nodes have not been partitioned to the JsExecutionProvider` (verified in Chrome via chrome-devtools MCP on onnxruntime-web 1.27, fp32 build). A try-capture-then-fallback wrapper works, but it wastes a full failed session initialization on every cold start for a failure that is a property of the graph, not the device, so it was reverted rather than shipped. To make capture viable, re-export the model so the graph partitions fully onto the WebGPU EP (constant-fold the shape subgraphs, e.g. with onnx-simplifier, in the training repo) and confirm the init error is gone before wiring it back in. When re-attempting, the session options alone are not enough: a capture session rejects CPU-located tensors at `run()` (`External buffer must be provided...`), so the input must live in one persistent GPU buffer written each frame with `device.queue.writeBuffer` and wrapped once via `Tensor.fromGpuBuffer` (device from `await env.webgpu.device` after session creation), outputs are forced to `gpu-buffer` location and must be read with `await tensor.getData(true)`, and incompatibility can also surface on the first run rather than at session creation, so keep a fallback path to a plain WebGPU session.
