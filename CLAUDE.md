...
- **Raw onnxruntime-web on purpose, never the Transformers.js `pipeline()`.** The head is a `1 + C`-wide sigmoid, one slot per class the checkpoint was trained on plus an unused background slot at index 0, and the pipeline's softmax-with-background DETR decode drops every real detection.
