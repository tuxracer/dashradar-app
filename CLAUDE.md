...
- `src/context/DetectionContext/`: worker lifecycle and frame-pump state machine; pacing, periodic worker recycle, crash-sentinel heartbeat, contact/saved-frame state, ref-backed debug snapshot.
...
- `src/lib/`: React-free domain modules, one directory each: `detection`, `detectionModels`, `detectionTracker`, `onnxMetadata`, `radarSignal`, `radarAudio`, `camera`, `crashSentinel`, `videoFileDrop`, `pwaInstall`, `saveFrame`, `serviceWorker`, `scanClock`, `timingHistory`, `browserEngine`, `appUpdate`, `resetAppData`, `wakeLock`, and friends.
