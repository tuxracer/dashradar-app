...
- **`src/lib/backendSafeMode/`**: React-free WASM safe mode. When the crash sentinel classifies the previous session as a crash that happened on the WebGPU backend, `src/instrument.ts` calls `armWasmSafeMode()` (outside the DNT gate; it is a local stability decision, not telemetry). While armed, `DetectionContext` posts every `load` with `forceWasm: true` (the worker cannot read localStorage, so the flag rides on the message) and the worker skips the GPU probe and runs the int8 WASM build; the debug overlay's engine row reads "(safe mode)" and the backend probe carries `safeMode`. The record is keyed to the release (`SAFE_MODE_RELEASE`): it is not consumed by reading, so it holds for the rest of the release, and a new deploy disarms it so each build retries WebGPU once. A one-shot flag would oscillate crash/clean/crash on alternating launches.
