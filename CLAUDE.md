...
- Tests: **vitest** + **@testing-library/react** (jsdom, which can't run the worker, inference, or the camera)

## Model

A custom **RF-DETR Small** checkpoint fine-tuned on Las Vegas Metro police vehicles, published at [`tuxracer/las-vegas-metro-rfdetr-small`](https://huggingface.co/tuxracer/las-vegas-metro-rfdetr-small) and trained/exported from the sibling repo `~/Development/las-vegas-metro-rfdetr-small`. Mobile WebGPU is the only target and the only execution path, so a bad export has nothing to catch it. `src/lib/detectionModels` defines the uniform entry shape (`id`, Hugging Face `owner` and `slug`, pinned `revision`, repo-relative `file`). `DEFAULT_MODEL` is the one entry a build ships with (`model_fp16.onnx`, ~57 MB, mixed precision); `knownModels()` is that default plus whatever a developer has added, persisted under the `models` `localStorage` key. Adding a model from the developer picker means pasting a Hugging Face URL: the app resolves it to a pinned entry, then proves it by downloading and running it once in a throwaway worker before registering it, so an incompatible checkpoint fails right there instead of after a reload; that trial load is also the cache fill. Registering an entry in code stays the way a build's own default changes, and a re-export is still a one-line revision bump.
...
- **Every `await` is a teardown window: re-check before committing the result.** Anything can change while an async acquisition is pending, including the caller deciding it no longer wants the result. After the await, re-validate the state that justified starting (a cancelled flag, the acquired flag, the live subscription) before storing the result or firing a side effect, and dispose the fresh resource when the answer is no. This matters most for resources that outlive the function when leaked: wake locks, media streams, ImageBitmaps, workers. The wake-lock manager once stored a sentinel granted after release() had already run, keeping the screen awake after scanning stopped; the `acquired` re-check in `src/lib/wakeLock`, the `cancelled` flag in `CameraView`, and `captureFrame`'s teardown flag in the detection engine are the pattern to copy.
