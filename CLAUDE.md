...
- `src/lib/`: React-free domain modules, one directory each: `detection`, `detectionModels`, `detectionTracker`, `autoZoom`, `radarSignal`, `radarAudio`, `camera`, `crashSentinel`, `videoFileDrop`, `pwaInstall`, `saveFrame`, `serviceWorker`, `scanClock`, `timingHistory`, `browserEngine`, `appUpdate`, `resetAppData`, `wakeLock`, and friends.
