...
| `shipping-a-model`     | changing the checkpoint, the ONNX export, the signature, or `MODEL_REVISION`           |
...
A custom **RF-DETR Small** checkpoint fine-tuned on Las Vegas Metro police vehicles, published at [`tuxracer/las-vegas-metro-rfdetr-small`](https://huggingface.co/tuxracer/las-vegas-metro-rfdetr-small) and trained/exported from the sibling repo `~/Development/las-vegas-metro-rfdetr-small`. Mobile WebGPU is the only target and the only execution path, so a bad export has nothing to catch it. `MODEL_URL` (`src/workers/detection/consts.ts`) streams `model_fp16.onnx` (~57 MB, mixed precision) from a pinned `MODEL_REVISION`, never `main`. `MODEL_CLASSES`, in the same file, is what the head actually emits: one entry per class the checkpoint was trained on, in head-index order, each with the label the model outputs and how the HUD displays and colors it. A new checkpoint that adds or reorders classes changes `MODEL_CLASSES` and `MODEL_REVISION` together, the only place a class is named. That does not make a new class free: `hudSignal` takes the max score across every detection regardless of class, so a new class immediately drives the dial, the `ALERT_THRESHOLD` ring, and the beeper, with no per-class alert concept to opt it out (a `person` class, for instance, would beep and flash red at every pedestrian). Read the `shipping-a-model` skill before changing the checkpoint, the export, the signature, or the revision.
