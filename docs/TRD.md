...
| --- | --- | --- | --- |
| WebGPU | A GPU adapter **and** device can actually be acquired, and the adapter exposes `shader-f16`: `resolveBackend()` awaits `navigator.gpu.requestAdapter()`, checks `adapter.features.has("shader-f16")`, then awaits `adapter.requestDevice()` (`src/workers/detection/index.ts`) | `model.onnx` (full-precision fp32, ~118 MB) | Faster; fp32 not fp16, because onnxruntime-web's WebGPU GridSample kernel is broken for fp16 (§4.1); verified end-to-end in Chrome via chrome-devtools MCP |
| WASM | No usable WebGPU: `navigator.gpu` is absent, `requestAdapter`/`requestDevice` returns nothing or throws, or the adapter lacks `shader-f16` | `model_int8.onnx` (int8 dynamic quant, fp32 I/O, ~35 MB) | Universal fallback; the smaller int8 build keeps the CPU path usable |
...
The `shader-f16` gate exists for the model roadmap, not for the current fp32 build (which runs without the feature). Any fp16 tensor in a model graph makes onnxruntime-web require `shader-f16` at session creation, so requiring it up front keeps the backend choice a two-way split: the webgpu URL can move to an fp16 or mixed-precision build (§4.1) without adding a third fp32-fallback branch or a failed-session double download, at the cost of sending the rare WebGPU-without-`shader-f16` device down the WASM path. The debug overlay's `shader-f16` row reports the verdict per device.
...
4. On the `detections` reply, `DetectionContext` decrements `inFlightRef`, promotes `captureOrientationRef` to `referenceOrientationRef` (the pose `getMotionDelta()` measures against, since these boxes were computed from a frame taken at that pose), runs `toRoadDetections` (`src/lib/detection`), passes the result through the `detectionTracker` coasting smoother (§5), then `buildHudModel` (`src/lib/detection`) to produce the `HudModel` the UI renders, records a result timestamp for the FPS estimate, and re-primes the pump via `schedulePacedFrame`.
...
10. FPS is a rolling average over the last `FPS_SAMPLE_SIZE` (10) result timestamps; a same-millisecond pair of results is skipped rather than producing a divide-by-zero reading.
...
| `detections` | `detections: RawDetection[]`, `timing: { preprocessMs, inferenceMs, decodeMs }`, `crop?: { image: ImageBitmap, detectionIndex: number }` | Decoded output for one frame, boxes normalized 0-1 (xyxy), the worker's own per-stage timing for the debug overlay, and an optional cutout of the frame's highest-scoring detection |
...
The fix is to serve the full-precision `model.onnx` on WebGPU, so the multiply is `f32 * f32` and the shader compiles. This was verified in Chrome via chrome-devtools MCP: with `model_fp16.onnx` the console filled with thousands of `Invalid ComputePipeline "GridSample"` validation errors; with `model.onnx` that count drops to zero and inference produces detections. The cost is download size (~118 MB fp32 vs ~64 MB fp16), accepted because a fast-but-wrong detector is useless for this app; a future mixed-precision export (fp16 weights with the GridSample nodes kept in fp32) could reclaim the size. `resolveBackend()` already gates WebGPU on the `shader-f16` adapter feature (§2) in preparation for such a build, so switching the webgpu URL to it needs no backend-selection change. Do not move WebGPU to an export with fp16 GridSample nodes without confirming onnxruntime-web has fixed the fp16 GridSample shader (still broken as of onnxruntime-web 1.27); the mixed-precision route sidesteps the bug by keeping those nodes fp32, but still needs the full verification pass (zero GridSample WGSL errors in a real Chrome run, detections matching fp32 on the reference image).

### 4.1.1 WebGPU graph capture (wired, disabled)

onnxruntime-web's graph capture (`enableGraphCapture: true`) records the model's kernel dispatches on the first run and replays them on later runs, cutting the per-frame CPU overhead of dispatching RF-DETR's hundreds of small kernels (the model repo measured ~18 vs ~21 ms/frame on a desktop GPU). The worker has the full wiring behind the `WEBGPU_GRAPH_CAPTURE` flag (`consts.ts`), off by default:
...
The flag is off because capture still cannot initialize with the current export. Capture requires every graph node partitioned onto the WebGPU EP. The v1.6 onnxslim re-export removed the shape subgraphs that originally tripped this check, but the graph retains one `TopK` node, and onnxruntime-web 1.27 has no WebGPU (JSEP) kernel for TopK, so ORT assigns it to the CPU EP and session creation fails deterministically on every device (`This session cannot use the graph capture feature ... all compute graph nodes have not been partitioned to the JsExecutionProvider`). Verified in Chrome via chrome-devtools MCP against both v1.6 WebGPU-capable builds (fp32 and fp16). Enabling the flag today would waste a full failed fp32 session initialization on every WebGPU cold start. To enable for real: re-export without TopK (the worker's decode scans all 300 queries itself, so a raw per-query export loses nothing) or wait for a WebGPU TopK kernel in onnxruntime-web, then flip the flag and confirm the overlay row reads "on" with detections matching the uncaptured baseline.
