...
src/
  ...
  lib/
    ...
    detectionTracker/           # temporal persistence gate between toRoadDetections and buildHudModel: greedy IoU matching, confirm-after-persistMs, coast-on-miss (pure step function + stateful factory)
...
4. On the `detections` reply, `DetectionContext` decrements `inFlightRef`, promotes `captureOrientationRef` to `referenceOrientationRef` (the pose `getMotionDelta()` measures against, since these boxes were computed from a frame taken at that pose), runs `toRoadDetections` (`src/lib/detection`), passes the result through the `detectionTracker` persistence gate (§5), then `buildHudModel` (`src/lib/detection`) to produce the `HudModel` the UI renders, records a result timestamp for the FPS estimate, and immediately calls `sendFrame()` again.
...
### Persistence gate (`src/lib/detectionTracker`)

A road-filtered detection isn't shown right away. `src/lib/detectionTracker` sits between `toRoadDetections` and `buildHudModel` and gates each detection on temporal persistence before it reaches the HUD, so a single-frame false positive never flashes a box onto the screen. Pipeline position:

```
worker → toRoadDetections (confidence >= 0.7) → detectionTracker (persist >= 500ms) → buildHudModel
```

The module is React-free and stateful, split into a pure step function and a stateful wrapper around it: `stepTracker(state, detections, nowMs, config)` runs one frame of the gate and returns the next `TrackerState` plus the `visible` (confirmed) detections; `createDetectionTracker()` wraps it in a small factory exposing `.update(detections, nowMs)`, which is what `DetectionContext` calls once per frame (§4, `performance.now()` supplies `nowMs`). `initialTrackerState()` gives an empty starting state, and `iou(a, b)` is the shared intersection-over-union helper both the tracker and its tests use.
...
- A **pending** (not yet confirmed) track must be matched every frame or it is dropped. This is what suppresses a single-frame blip: a detection that doesn't reappear on the very next frame never confirms.
- A pending track **confirms** once it has been matched while at least `PERSIST_MS` (`500` ms) old, measured from the track's first sighting.
- A **confirmed** track coasts through up to `MAX_MISSES` (`2`) consecutive unmatched frames before it is dropped, so the box doesn't flicker off when the model briefly loses the object for a frame or two.
- An unmatched detection in the current frame starts a brand-new pending track.

Only confirmed tracks are returned as `visible` detections for `buildHudModel` to shape. The debug overlay's `detections` row reflects all three pipeline stages, read left to right as `confirmed / filtered / raw` (`DebugSnapshot.confirmedCount` / `.filteredCount` / `.rawCount`, §4).
...
- **`src/lib/detectionTracker`**: `iou` against identical, disjoint, and partially overlapping boxes; `stepTracker` not showing a detection on its first sighting, confirming it once it persists past `PERSIST_MS`, dropping a single-frame blip instead of confirming it, coasting a confirmed track through `MAX_MISSES` frames before dropping it, keeping one track matched as its box drifts (IoU match), and greedily matching two detections to two separate tracks without double-claiming; `createDetectionTracker` holding state across calls.
...
- **`src/context/DetectionContext`**: the full status machine against an injected fake worker (the `createWorker` test seam), including the one-frame-in-flight invariant across a fast `stop()`-then-`start()` (a regression test for a real race that was fixed), a detection's first frame being held back by the persistence gate instead of reaching the HUD immediately, retrying after a `createImageBitmap` failure, and the FPS calculation staying finite. Because the `load` message is now deferred to a microtask (`import.meta.env.PROD` is false in tests, so it resolves immediately), the "starts loading on mount" test awaits it with `waitFor` rather than asserting synchronously.
