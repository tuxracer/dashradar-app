...
6. **Pacing floor**: `schedulePacedFrame` keeps captures at least `MIN_FRAME_INTERVAL_MS` (125 ms, ~8 Hz) apart. A result that returns faster than the floor schedules the next capture on a timeout for the remainder instead of sending immediately; a result slower than the floor sends immediately, unchanged. This caps the detection rate on fast (WebGPU) devices so a dash-mounted phone idles its GPU between frames rather than running inference back-to-back and thermal-throttling; the coasting tracker and motion stabilization cover the gaps between results. The pacing timer is cleared on `stop()`, worker errors, and provider teardown so a stale timer can't pump a stopped session.
