...
| `detect` | `frame: ImageBitmap` (transferred), `includeFrame?: boolean`, `centerCrop?: boolean`, `confidenceThreshold?: number` | Run one frame through the model; `includeFrame` asks for the full frame back on the response for debug-mode saving; `centerCrop` (default true when omitted) selects center-crop vs squish preprocessing (§4 step 3); `confidenceThreshold` (default `CONFIDENCE_THRESHOLD`, 0.5, when omitted) sets the minimum score `decodeDetections` keeps, carrying the developer confidence-threshold setting to the worker's decode |
...
and validated on read with `isPersistedSettings`. Six options:
...
`developerOptions` (default off) is the master switch for the four
development-only options: `showDebug`, `throttleInference`,
`centerCropFrames`, and `confidenceThreshold`. `SettingsScreen` renders their
rows only while it is on, and `SettingsProvider` reports all four at their
`DEVELOPER_OPTIONS_OFF` values while it is off, so a tweak left enabled cannot
alter a normal drive. The gate lives in the provider, not in each consumer:
`useSettings()` returns the already-gated effective value, so
`DetectionContext` reads a bare
`throttleInference`/`centerCropFrames`/`confidenceThreshold` and
`DebugOverlay` a bare `showDebug`. The stored values are left untouched while
the switch is off, so turning it back on restores the tweaks rather than
resetting them.
...
`snapConfidence`). It is gated the same way as the other three:
