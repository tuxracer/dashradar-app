...
6. **Pacing**: `schedulePacedFrame` keeps captures at least `MIN_FRAME_INTERVAL_MS` (125 ms, ~8 Hz) apart and always rests at least `PACING_REST_RATIO` (0.5) of the last result's round trip before the next capture. On fast (WebGPU) devices the absolute floor dominates: a result that returns faster than 125 ms schedules the next capture on a timeout for the remainder, capping the detection rate so a dash-mounted phone idles its GPU between frames rather than running inference back-to-back and thermal-throttling. On devices slower than the floor the rest ratio takes over: a 1.2 s round trip is followed by at least 600 ms of idle, capping the inference duty cycle at roughly two thirds instead of letting slow phones run the GPU flat out (which compounds, since sustained throttling makes inference slower still). The coasting tracker and motion stabilization cover the gaps between results. The pacing timer is cleared on `stop()`, worker errors, and provider teardown so a stale timer can't pump a stopped session.
