...
| `detections` | `detections: RawDetection[]`, `timing: { preprocessMs, inferenceMs, decodeMs }`, `crop?: { image: ImageBitmap, detectionIndex: number }`, `frame?: Blob`, `fingerprint?: number` | Decoded output for one frame, boxes normalized 0-1 (xyxy), the worker's own per-stage timing for the debug overlay, an optional cutout of the frame's highest-scoring detection, an optional JPEG of the full frame when the request set `includeFrame`, and a content fingerprint of the decoded frame for camera-stall detection |
