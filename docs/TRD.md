...
| `detect` | `frame: ImageBitmap` (transferred), `includeFrame?: boolean`, `includeThumbnail?: boolean`, `centerCrop?: boolean`, `confidenceThreshold?: number` | Run one frame through the model; `includeFrame` asks for the full frame back on the response for frame saving; `includeThumbnail` asks for a thumbnail of the model's input on scans with no detection to crop; `centerCrop` (default true when omitted) selects center-crop vs squish preprocessing (§4 step 3); `confidenceThreshold` (default `CONFIDENCE_THRESHOLD`, 0.5, when omitted) sets the minimum score `decodeDetections` keeps, carrying the developer confidence-threshold setting to the worker's decode |
...
- `first_inference` `{ backend }` fires on the **first** `detections` reply of the page load (gated by `firstInferenceTrackedRef`), and never again for that session. It is the end-of-funnel event: reaching it means the intro was dismissed, the in-app permission ask was accepted, camera permission was granted, the stream started, the model loaded, and a frame completed inference. Every earlier gate (`intro_start`, `camera_prompt_allow`/`camera_prompt_decline`, the `error` codes, `model_ready`) has its own drop-off, so this is the single event that says all of them were cleared. Once-per-session on purpose: the pump produces a result every couple of seconds, so a per-frame event would be thousands of events per drive and useless as a rate.
...
- `timing_round_trip` `{ seconds }` and `timing_inference` `{ seconds }` fire together, once per session, the moment the rolling timing window (§4, `src/lib/timingHistory`) first reaches ten scans. Each carries that series' median in seconds on the half-second grid, so the two events give the fleet-wide distribution of how fast scans actually complete on real devices, which is the number the pacing floor and the thermal budget (§1) are set against. Ten scans in is late enough to be past the first-run costs (session compile, a cold GPU) and early enough that most sessions reach it. Once per session on purpose, guarded by the `dashradar:timingsReported` sessionStorage mark inside `takeTimingReport`, not by a ref: a per-window event would fire every ten scans for the length of a drive, and re-firing per worker recycle would bias the sample toward long drives. A session whose storage cannot record the mark sends nothing rather than risk an event per scan.
...
and validated on read with `isPersistedSettings`. Nine options:
...
the two diverge on the three display options (`showDebug`, `frameThumbnails`,
`saveFrames`): each has an off-switch value of false and a stored default of
true, so turning Developer options on brings all three up with no second tap
while a normal drive never sees them.
...
Developer options is turned on and on from that moment. `frameThumbnails`
(default on under the master switch) makes the contact card show a thumbnail of
...
message as `includeThumbnail`. `saveFrames` (same default) adds the card's SAVE
...
`autoSaveFrames` (**default off**, the one developer option that does not
default on under the master switch) downloads a detection's full frame the
moment it arrives, with no tap on SAVE. It exists for collecting training data
...
- **Status bar** (`StatusBar`, safe-area aware): `DASHRADAR.APP` wordmark top-left; the `SettingsButton` gear top-right. The `GPU` / `CPU` engine readout lives in the full-screen `SettingsScreen`, not the bar.
