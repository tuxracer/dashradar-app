...
| Analytics | **`@vercel/analytics`** | `inject()` in `src/main.tsx`. Anonymous events only, never camera frames, detection boxes, or location: page views, UI events (`intro_start`, `settings_open`, `share_click`), health events (`backend_resolved`, `model_ready`, `error`), and a debounced `police_detected` sighting count. See §4.3. |
