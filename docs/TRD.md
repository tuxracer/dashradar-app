...
`useDetection()` hands out the engine's published snapshot (status, download progress, the `HudModel`, `scan` with the frame's raw per-frame detections, the current `contact`, an error code) plus `start(video)` / `stop()` and `getDebugSnapshot()`. The debug snapshot is read on demand, never published: per-frame timing through state would re-render every consumer for numbers nobody is showing.
