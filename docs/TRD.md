...
- `first_inference` `{ backend }` fires on the **first** `detections` reply of the page load (gated by `firstInferenceTrackedRef`), and never again for that session. It is the end-of-funnel event: reaching it means the intro was dismissed, camera permission was granted, the stream started, the model loaded, and a frame completed inference. Every earlier gate (`intro_start`, the `error` codes, `model_ready`) has its own drop-off, so this is the single event that says all of them were cleared. Once-per-session on purpose: the pump produces a result every couple of seconds, so a per-frame event would be thousands of events per drive and useless as a rate.
...
`autoSaveFrames`, `throttleInference`, `centerCropFrames`, `zoom2x`, and
...
`frameThumbnails`/`saveFrames`/`autoSaveFrames`/`throttleInference`/`centerCropFrames`/`zoom2x`/`confidenceThreshold`
...
`zoom2x` (default off, even under Developer options) halves the centered
square the worker feeds the model, so the same 512x512 input covers half the
field of view and a distant vehicle occupies twice the linear size in the
input grid. It is a digital crop rather than the camera's own zoom: native
zoom (`MediaStreamTrack.applyConstraints({ advanced: [{ zoom }] })`) does not
...
cleanly. `DetectionContext` mirrors the setting into the `zoom` crop factor
(`ZOOM_2X`/`ZOOM_OFF`) it posts with every `detect`, and it is gated like the
rest, so a stored true can never narrow a normal drive.
