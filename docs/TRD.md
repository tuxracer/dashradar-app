...
The fix is to serve the full-precision `model.onnx` on WebGPU, so the multiply is `f32 * f32` and the shader compiles. This was verified in Chrome via chrome-devtools MCP: with `model_fp16.onnx` the console filled with thousands of `Invalid ComputePipeline "GridSample"` validation errors; with `model.onnx` that count drops to zero and inference produces detections. The cost is download size (~118 MB fp32 vs ~64 MB fp16), accepted because a fast-but-wrong detector is useless for this app; a future mixed-precision export (fp16 weights with the GridSample nodes kept in fp32) could reclaim the size. `resolveBackend()` already gates WebGPU on the `shader-f16` adapter feature (§2) in preparation for such a build, so switching the webgpu URL to it needs no backend-selection change. Do not move WebGPU to an export with fp16 GridSample nodes without confirming onnxruntime-web has fixed the fp16 GridSample shader (still broken as of onnxruntime-web 1.27); the mixed-precision route sidesteps the bug by keeping those nodes fp32, but still needs the full verification pass (zero GridSample WGSL errors in a real Chrome run, detections matching fp32 on the reference image). Caveat: everything in this section was observed under JSEP, the root import's WebGPU implementation. The worker has since moved to the native C++ WebGPU EP (`onnxruntime-web/webgpu`, §4.1.1), a completely separate kernel implementation, where the model repo verified the v1.5+ mixed-precision fp16 build runs correctly; re-check any conclusion here against the native EP before relying on it.
