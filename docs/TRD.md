...
**Class table and threshold.** A class is defined once, in the `classes` of its model's registry entry (§7): label, display label, and category, in head-index order. It belongs to the checkpoint rather than to either consumer, and both the worker's decode and this module read the registry, so neither has to import the other to name a class. There is no allowlist any more: every label the table names is shown, and a label it doesn't (worker and HUD disagreeing on a table they both read) still surfaces, under an `unknown` category, rather than vanishing. `CONFIDENCE_THRESHOLD` (0.5) is applied in the worker's decode and again defensively when detections are enriched; both take it as a parameter, so the developer slider changes both without a worker reload. `SIGNAL_FLOOR` is the same value and moves with it, or the meter wastes range on scores that can never reach the HUD.
