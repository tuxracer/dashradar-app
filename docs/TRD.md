...
src/
  ...
  components/
    ...
    DevVideoView/               # dev-only stand-in for CameraView, same props plus src: plays DEV_VIDEO_URL as the detection feed and doubles as a visible, controllable corner player (§4.3)
    ...
    RecoveryOverlay/            # full-screen "Reconnecting camera..." state shown while DetectionContext's recovering is true; sits above the radar meter, below ModelLoadScreen/ErrorScreen
...
type DetectionContextValue = {
  ...
  recovering: boolean;                      // true while the camera feed is being re-acquired after a detected stall
...
`frameThumbnail`, when present, is a downscaled `ImageBitmap` of the whole inference frame, sent only in debug mode (`includeFrame`) on a scan that had **no** top detection to crop. `createFrameThumbnail` (`src/workers/detection/index.ts`) resizes the frame so its long edge is at most `CROP_MAX_EDGE` (320px, never upscaled, matching the crop's sizing) and, like the crop, is **transferred** rather than cloned. It is mutually exclusive with `crop`: a frame with a top detection sends `crop` instead. This is what lets the contact card show what every scan saw in debug mode, not just the scans that detected something. Best-effort like the crop: a `createImageBitmap` failure just omits it.
...
`App`'s `RadarScreen` renders `DevVideoView` instead of `CameraView` whenever `DEV_VIDEO_URL` is set. `DevVideoView` (`src/components/DevVideoView`) takes the same props as `CameraView` plus `src`, and plays the clip on a loop. Unlike `CameraView`'s permanently `opacity-0` element, its `<video>` is the visible UI: rendered with native `controls` in a fixed corner of the screen, so the clip can be paused and scrubbed by hand. This mode only ever runs on a desktop browser, so the player is sized for mouse use, not the dash-mount touch-target rules the rest of the app follows. On mount it calls `video.play()`; success reports the element through `onStream` (the same `start(video)` wiring `CameraView` uses) and wires the `resize` listener for `onVideoResize`, while a rejected `play()` reports a `CameraError` with code `NO_CAMERA` through `onError`, so a blocked autoplay still renders through the existing `ErrorScreen` path rather than a bespoke one.
...
`showDebug` (default false) gates the `DebugOverlay` panel; it doesn't change
what detection does. `radarAudio` (default on) gates the radar-detector
...
ALERT. `SettingsProvider` wraps the app outside `DetectionProvider`;
...
`CONFIDENCE_THRESHOLD` is `0.7`. It's applied twice: once in the worker's `decodeDetections` (a query is emitted only when `sigmoid(policeLogit) >= CONFIDENCE_THRESHOLD`), and again defensively in `toRoadDetections` (`candidate.score < CONFIDENCE_THRESHOLD` is dropped) so a low-confidence result can never slip through. `SIGNAL_FLOOR` (`src/lib/radarSignal/consts.ts`), the score the radar-detector-style meter treats as zero signal, is also `0.7` and is meant to move in lockstep with `CONFIDENCE_THRESHOLD`: since the road filter already drops anything below the confidence threshold, the meter would otherwise waste part of its range on scores that can never reach the HUD.
