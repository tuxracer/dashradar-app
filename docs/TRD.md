...
**Class table and threshold.** `MODEL_CLASSES`, in the worker's consts next to `MODEL_URL`, is the one place a class is defined: label, display label, and category, in head-index order. It lives there because it ships with the checkpoint and because this module already imports from the worker, so the reverse import would cycle. There is no allowlist any more: every label the table names is shown, and a label it doesn't (worker and HUD disagreeing on a table they both read) still surfaces, under an `unknown` category, rather than vanishing. `CONFIDENCE_THRESHOLD` (0.5) is applied in the worker's decode and again defensively when detections are enriched; both take it as a parameter, so the developer slider changes both without a worker reload. `SIGNAL_FLOOR` is the same value and moves with it, or the meter wastes range on scores that can never reach the HUD.
...
**HUD shaping.** `buildHudModel` computes `top`, the highest-scoring detection, which the dial's percentage and the status word's class name both read. The contact card and its direction readout instead come from the worker's own score-based pick (`topDetectionIndex`). Both pick by score but read different data, the card the raw detections of the frame just decoded and the dial the coasted tracker set, so they agree on a live frame and drift apart while a track coasts. Auto zoom reads the whole tracked set, not one detection. `nearest` (largest box by area), `near`, and `others` are computed but unused. `mapBoxToViewport`, from the same HUD, maps both `scanRegionBox` (the model's centered crop, narrowed by zoom) and each box for the detection view, assuming `object-fit: cover`.
