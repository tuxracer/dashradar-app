...
| Fonts | **Rajdhani** | Self-hosted via `@fontsource/rajdhani` (weights 500/600/700), imported in `src/main.tsx` as the `latin-`/`latin-ext-` subset entrypoints. The unqualified `500.css`/`600.css`/`700.css` pull every subset, and Rajdhani is a Devanagari-and-Latin typeface: that was 234 KB of the 319 KB of font bytes, precached on every install for glyphs an uppercase-Latin HUD never renders. Only font in the app. |
