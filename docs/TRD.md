...
flowchart TB
    ...
    W --> F["toRoadDetections<br>(class-table enrich, confidence floor)"]
...
**HUD shaping.** `buildHudModel` splits one frame's detections two ways: `nearest` is the largest box by normalized area, the one the contact card crops, the direction readout points at, and auto zoom steps on; `top` is the highest-scoring detection, the one the dial's percentage and the status word's class name both read, so the two never disagree. `others` is everything except `nearest`, so `top` usually appears there too. `mapBoxToViewport`, from the same HUD, maps both `scanRegionBox` (the model's centered square crop, narrowed by zoom) and each detection's box for the detection view, so it still assumes the video is rendered `object-fit: cover`.
