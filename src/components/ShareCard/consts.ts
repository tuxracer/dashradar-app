...
/**
 * Pre-rendered QR code for SHARE_URL as SVG path data, quiet zone included.
 * Generated once rather than at runtime, so the app ships no library code for a
 * URL that never changes. To regenerate:
 *   pnpm dlx qrcode "<url>" -t svg -e M
 * then copy the stroke path's `d` and update QR_VIEW_BOX_SIZE.
 */
